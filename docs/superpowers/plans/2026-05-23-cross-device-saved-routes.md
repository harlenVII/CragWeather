...
**Model:** claude-haiku-4-5-20251001 — mechanical schema add, migration generated by drizzle, code fully specified.
...
**Model:** claude-haiku-4-5-20251001 — pure mechanical copy of validator + tests, all code in plan.
...
**Model:** claude-haiku-4-5-20251001 — handler and tests fully specified, mechanical TDD copy.
...
**Model:** claude-haiku-4-5-20251001 — handler and tests fully specified.
...
**Model:** claude-haiku-4-5-20251001 — tests only (handler already implemented in Task 4), pure copy from plan.
...
**Model:** claude-sonnet-4-6 — substantial hook rewrite with stateful interactions (refs, async effects, fetch mocking), and 9 new tests covering subtle behaviors. Beyond a mechanical copy.
...
**Model:** claude-haiku-4-5-20251001 — single npm install command + commit.
...
**Model:** claude-sonnet-4-6 — new component with QR rendering, clipboard API, two-mode (linked vs unlinked) rendering, and a 5-test suite using `userEvent` + fetch mocking. Code is provided but layered enough to warrant Sonnet judgment.
...
**Model:** claude-sonnet-4-6 — full component rewrite plus potential updates to `HomePage.test.tsx` (judgment-driven: only modify if existing assertions break). Not purely mechanical.
...
**Model:** claude-sonnet-4-6 — two files (server page + client component), router mocking, conditional warning rendering. Provided code, but multi-file coordination warrants Sonnet.
...
**Model:** claude-haiku-4-5-20251001 — pure CSS append, all rules in the plan, plus a brief visual sanity check in the browser.
...
**Model:** claude-sonnet-4-6 — requires browser interaction, multi-step verification, judgment about whether observed behavior matches intent.
